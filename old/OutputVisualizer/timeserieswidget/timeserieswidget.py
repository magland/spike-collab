...
class TimeseriesWidget:
    def __init__(self,reader,channels=None,trange=None):
        self._reader=reader
        self._samplerate=reader.sampleRate()
        ...
        self._vspacing=self._mean_channel_std*12
    ...
    def _initialize_stats(self):
        ...
        chunk0=self._reader.getChunk(channels=range(1,M+1),trange=[0,np.minimum(N,10000)])
        for m in range(1,M+1):
            self._channel_stats[m]=self._compute_channel_stats_from_data(chunk0[m-1,:])
        self._mean_channel_std=np.mean([self._channel_stats[m]['std'] for m in range(1,M+1)])
